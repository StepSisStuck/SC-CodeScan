...
const validationFn = require('../validation/validateFn.js');
...
app.use(morgan(
  '{"date":":date[iso]","method":":method","url":":url","status":":status","ip":":remote-addr","user-agent":":user-agent"}',
    ...
    productDB.updateProduct(name, description, categoryid, brand, price, req.params.productid, (err, results) => {
        ...
        //No error, response with productid
            ...
            if (results.affectedRows < 1)
                ...
                console.info(`[INFO] User ${req.userid} (${req.type}) updated product ID: ${req.params.productid} with data: ${JSON.stringify(req.body)}`);
...
app.post('/user/login', function (req, res) {
    ...
    userDB.loginUser(username, password, function (err, result, token) {
        if (!err) {
            ...
            res.status(500);
            console.warn(`[WARN] Failed login for user: ${username}`); //log failed attempt in app.js
...
app.post('/users', validationFn.validateUser, (req, res) => {

    var { username, email, contact, password, profile_pic_url } = req.body;
    //passes extracted values along the role, Customer
    userDB.addNewUser(username, email, contact, password, "Customer", profile_pic_url, (err, results) => {
        if (err) {
            if (err.code == "ER_DUP_ENTRY") //data validation for if user name is taken
...
app.get('/users', (req, res) => {
    ...
    userDB.getAllUser((err, results) => {
        ...
        //No error, response with all user info
            ...
            res.status(200).json(validationFn.sanitizeResult(results)) //sanitize output
...
app.get('/users/:id', (req, res) => {
    ...
    userDB.getUser(req.params.id, (err, results) => {
        ...
        //No error, response with user info
            ...
            res.status(200).json(validationFn.sanitizeResult(results)[0]) //sanitize output
...
app.post('/product', verifyToken, (req, res) => {
    ...
    productDB.addNewProduct(name, description, categoryid, brand, price, (err, results) => {
        ...
        //No error, response with productid
            ...
            console.info(`[INFO] User ${req.userid} (${req.type}) created product: ${JSON.stringify(req.body)}`); 
            //to log creation of new product
...
app.delete('/product/:id', verifyToken, (req, res) => {
    ...
    productDB.deleteProduct(req.params.id, (err, results) => {
        ...
        if (err)
            ...
            console.info(`[INFO] User ${req.userid} (${req.type}) deleted product ID: ${req.params.id}`);
            res.status(204).end();
...
app.post('/product/:id/review/', verifyToken, validationFn.validateReview, (req, res) => {
    ...
    reviewDB.addReview(userid, rating, review, req.params.id, (err, results) => {
        ...
        //No error, response with reviewid
            ...
            res.status(200).json(validationFn.sanitizeResult(results)); //sanitize output before sending
